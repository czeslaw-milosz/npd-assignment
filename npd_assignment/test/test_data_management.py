...
def data_before_preprocessing():
    return (
        pd.read_csv("./data/emission_before_preprocessing.csv"),
...
def data_preprocessed():
    return (
        pd.read_csv("./data/emission_preprocessed.csv"),
...
class TestDataManager:
    ...
    def test_read_data(self, raw_data_paths, data_before_preprocessing):
        ...
        assert all(
            (
                x.equals(y)
                for x, y in zip(
                    (tmp[0], tmp[1], tmp[2]), data_before_preprocessing
                )
            )
        )
    ...
    def test_ensure_data_consistency(
        self, data, expected_year, expected_country
    ):
        dm = DataManager("", "", "")
        dm._emission_df, dm._gdp_df, dm._population_df = data
        dm._ensure_data_consistency()
        assert all(
            df["Year"].tolist() == expected_year
            for df in (dm._emission_df, dm._gdp_df, dm._population_df)
        )
        assert all(
            df["Country"].tolist() == expected_country
            for df in (dm._emission_df, dm._gdp_df, dm._population_df)
        )
